...
public class AlertsServiceTest {
	...
	public void testGetFireStationCoverage() {
		...
		List<FireStation> fireStations = Arrays.asList(new FireStation("1509 Culver St", "1"),
				new FireStation("123 Main St", "1"), new FireStation("456 Elm St", "2"));
		...
		MedicalRecord medicalRecord1 = new MedicalRecord("John", "Boyd", "03/06/1984", new ArrayList<>(),
				new ArrayList<>());
		MedicalRecord medicalRecord2 = new MedicalRecord("Jane", "Smith", "05/01/2015", new ArrayList<>(),
				new ArrayList<>());
		MedicalRecord medicalRecord3 = new MedicalRecord("Robert", "Doe", "02/05/2016", new ArrayList<>(),
				new ArrayList<>());
		when(medicalRecordDAO.getMedicalRecordByFullName("John", "Boyd")).thenReturn(medicalRecord1);
		when(medicalRecordDAO.getMedicalRecordByFullName("Jane", "Smith")).thenReturn(medicalRecord2);
		when(medicalRecordDAO.getMedicalRecordByFullName("Robert", "Doe")).thenReturn(medicalRecord3);
		...
		for (FireStationCoverage coverage : result) {
			...
			List<PersonCoverage> expectedPersons = persons.stream()
					...
					.map(person -> new PersonCoverage(person.getFirstName(), person.getLastName(), person.getAddress(),
							person.getPhone()))
					.collect(Collectors.toList());

			assertThat(coverage.getPersons().size()).isEqualTo(expectedPersons.size());
			assertThat(coverage.getPersons()).isEqualTo(expectedPersons);
			assertThat(coverage.getNumAdults() + coverage.getNumChildren()).isEqualTo(expectedPersons.size());
	...
	public void testGetFloodStations() {
		...
		// Assert
		assertThat(result).isNotNull();		
		assertThat(result.size()).isEqualTo(stationNumbers.size());
